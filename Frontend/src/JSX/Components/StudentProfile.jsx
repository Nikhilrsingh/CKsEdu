...
const StudentProfile = () => {
  ...
  const StatCard = ({ icon: Icon, title, value, color, bgColor }) => (
    <motion.div
      ...
      className={`${bgColor} p-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 border ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}
    >
  ...
  const GradeRow = ({ subject, credits, grade, status }) => (
    <motion.tr
      ...
      className={`border-b transition-colors ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}
    >
      <td className={`px-6 py-4 text-sm font-medium flex items-center ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>
      ...
      <td className="px-6 py-4">
        <span
          className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${grade.includes("A")
            ? darkMode
      ...
      <td className="px-6 py-4">
        <span
          className={`inline-flex items-center px-2 py-1 text-xs font-semibold rounded-full ${status === "Pass"
            ? darkMode
              ? "bg-green-900 text-green-200"
              : "bg-green-100 text-green-800"
            : darkMode
  ...
  const AchievementBadge = ({ icon: Icon, title, color }) => (
    <motion.div
      ...
      whileTap={{ scale: 0.95 }}
      className={`p-4 rounded-lg shadow-md hover:shadow-lg transition-all duration-300 border cursor-pointer ${darkMode
    ...
    <div className={`min-h-screen ${darkMode ? "dark" : ""}`}>
      <div className={`transition-colors duration-300 ${darkMode
        ...
        <div className="max-w-7xl mx-auto p-6 space-y-6">
          ...
          {/* Dark Mode Toggle - Removed since theme is now handled globally */}
        </div>
      </div>
    </div>
  );
